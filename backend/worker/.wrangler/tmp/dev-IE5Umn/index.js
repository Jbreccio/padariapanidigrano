...
async function firewall(context) {
  try {
    const { request } = context;
    if (!request) {
      console.error("\u{1F525} Firewall: request undefined");
      return null;
    }
...
var index_default = {
  async fetch(request, env, ctx) {
    ...
    if (payloadError) return addCorsHeaders(addSecurityHeaders(payloadError));
    ...
    try {
      ...
      if (wafBlock) {
        ...
        return addCorsHeaders(addSecurityHeaders(wafBlock));
      }
      ...
      const allowed = firewall(request);
      if (!allowed) {
        await logAttack(env, {
          type: "firewall",
          ip: clientIP,
        ...
        return addCorsHeaders(
          addSecurityHeaders(
            new Response("Blocked by firewall", { status: 403 })
          )
        );
      }
      ...
      if (isBot) {
        ...
        return addCorsHeaders(
          addSecurityHeaders(
            new Response("Bot detectado", { status: 403 })
          )
        );
      }
      ...
      if (!isPublicRoute && !isAuthRoute) {
        ...
        if (authResult?.error) {
          ...
          return addCorsHeaders(addSecurityHeaders(authResult.response));
        }
      ...
      if (pathname === "/api/auth/login" && request.method === "POST") {
        return addCorsHeaders(addSecurityHeaders(await fielLoginRoute(request, env)));
      }
      if (pathname === "/api/auth/verificar" && request.method === "GET") {
        return addCorsHeaders(addSecurityHeaders(await fielVerificarRoute(request, env)));
      }
      if (pathname === "/api/auth/verify-pin" && request.method === "POST") {
        return addCorsHeaders(addSecurityHeaders(await fielVerifyPinRoute(request, env)));
      }
      if (pathname === "/api/auth/reenviar-pin" && request.method === "POST") {
        return addCorsHeaders(addSecurityHeaders(await fielReenviarPinRoute(request, env)));
      }
      if (pathname === "/api/auth/verify-2fa" && request.method === "POST") {
        return addCorsHeaders(addSecurityHeaders(await fielVerify2faRoute(request, env)));
      }
      if (pathname === "/api/auth/reset-2fa-backup" && request.method === "POST") {
        return addCorsHeaders(addSecurityHeaders(await fielReset2faBackupRoute(request, env)));
      }
      if (pathname === "/api/auth/solicitar-reset-2fa" && request.method === "POST") {
        return addCorsHeaders(addSecurityHeaders(await fielSolicitarReset2faRoute(request, env)));
      }
      if (pathname === "/api/auth/confirmar-reset-2fa" && request.method === "POST") {
        return addCorsHeaders(addSecurityHeaders(await fielConfirmarReset2faRoute(request, env)));
      }
      if (pathname === "/api/auth/esqueci-senha" && request.method === "POST") {
        return addCorsHeaders(addSecurityHeaders(await fielEsqueciSenhaRoute(request, env)));
      }
      if (pathname === "/api/auth/confirmar-reset-senha" && request.method === "POST") {
        return addCorsHeaders(addSecurityHeaders(await fielConfirmarResetSenhaRoute(request, env)));
      }
...
// .wrangler/tmp/bundle-yZYs0j/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yZYs0j/middleware-loader.entry.ts
