...
async function salvarComBackup(env, kv, key, novoValor) {
...
__name(salvarComBackup, "salvarComBackup");
...
async function getSession(request, env) {
  try {
    ...
    const session = await env.KV_SESSIONS.get(`sess:${hash}`, "json");
...
async function rateLimit(request, env, limit = 10, windowSeconds = 60) {
  try {
    const ip = request.headers.get("CF-Connecting-IP") || "unknown";
    const key = `rate:${ip}`;
    let data = await env.KV_RATE.get(key, "json");
    const now = Date.now();
    if (!data) {
      data = { count: 0, start: now };
    }
    if (now - data.start > windowSeconds * 1e3) {
      data = { count: 0, start: now };
    }
    data.count++;
...
async function registrarTermoPublico(request, env, ctx) {
  try {
    ...
    const validarEmail2 = /* @__PURE__ */ __name((emailStr) => {
      const regex = /^[^\s@]+@([^\s@]+\.)+[^\s@]+$/;
      return regex.test(emailStr);
    }, "validarEmail");
    if (!validarEmail2(email)) {
...
__name(base32tohex, "base32tohex");
async function verifyTOTP(secret, token) {
  ...
  for (let i = -1; i <= 1; i++) {
    ...
    const code = (h[offset] & 127) << 24 | (h[offset + 1] & 255) << 16 | (h[offset + 2] & 255) << 8 | h[offset + 3] & 255;
...
async function fielRegisterRoute(request, env) {
  try {
    ...
    if (!validarEmail(email)) {
      return jsonResponse({ success: false, error: "E-mail inv\xE1lido" });
    }
    if (!validarCelular(celular)) {
      return jsonResponse({ success: false, error: "Celular inv\xE1lido (com DDD, m\xEDnimo 10 d\xEDgitos)" });
    }
    ...
    if (existe) {
      return jsonResponse({ success: false, error: "E-mail j\xE1 cadastrado" });
    }
...
async function fielVerify2faRoute(request, env) {
  try {
    ...
    const valid = await verifyTOTP(user.twofa_secret, codigo2FA);
    if (!valid) {
    ...
    const role = user.role || (user.id === 1 ? "admin" : "fiel");
    const token = crypto.randomUUID();
    const hash = await sha256(token);
    const expiresIn = 7 * 24 * 60 * 60 * 1e3;
    const expiresAt = Date.now() + expiresIn;
    await env.KV_SESSION.put(`sess:${hash}`, JSON.stringify({
      user: {
        id: user.id,
        nome: user.nome,
        email: user.email,
...
// src/routes/auth/admin_auth.js
async function adminVerificarRoute(request, env) {
  try {
...
__name(adminVerificarRoute, "adminVerificarRoute");
...
async function getInstagramToken(env) {
  ...
  if (!token) {
    ...
    if (token && env.KV_INSTAGRAM) {
      ...
      console.log("\u{1F4BE} Token salvo no KV_INSTAGRAM a partir da vari\xE1vel de ambiente");
...
async function refreshInstagramToken(env, motivo = "auto_refresh") {
  ...
  try {
    ...
    if (data.access_token) {
      ...
      console.error("\u274C Falha na renova\xE7\xE3o:", data);
      await registrarHistoricoToken(env, "FAILED", `${motivo}_failed`);
      return null;
    }
  } catch (error) {
    console.error("\u274C Erro na renova\xE7\xE3o:", error);
    return null;
  }
}
...
async function getInstagramFeed(env) {
  ...
  if (!IG_ID || !TOKEN) {
    console.error("\u274C Instagram: ID ou Token n\xE3o configurado");
    return [];
  }
  const url = `https://graph.facebook.com/v22.0/${IG_ID}/media?fields=id,caption,media_url,permalink,timestamp&access_token=${TOKEN}`;
  try {
    console.log("\u{1F50D} Buscando feed do Instagram...");
    const res = await fetch(url);
    const json = await res.json();
    if (json.error) {
      console.error("\u274C Erro na API do Instagram:", json.error);
      if (json.error.code === 190 && json.error.error_subcode === 463) {
    ...
    console.log(`\u2705 Instagram: ${finalPosts.length} posts encontrados`);
    return finalPosts;
  } catch (err) {
...
var index_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (SENSITIVE_ROUTES.some((route) => pathname === route)) {
        const isAllowed = await rateLimit(clientIP, env, 5, 60);
        if (!isAllowed) {
          const response2 = jsonResponse({ success: false, error: "Muitas tentativas. Aguarde 1 minuto." }, 429);
          return addCorsHeaders(addSecurityHeaders(response2));
        }
      }
      let user = null;
      const isPublicRoute = PUBLIC_ROUTES.some((route) => pathname === route) || pathname.startsWith("/api/auth/reset-2fa") || pathname.includes("reset2fa");
      if (!isPublicRoute) {
        user = await verifyToken(request, env);
        if (AUTH_ROUTES.some((route) => pathname === route || pathname.startsWith("/api/fiel/"))) {
          if (!user) {
            const response2 = jsonResponse({ success: false, error: "Token inv\xE1lido ou expirado" }, 401);
            return addCorsHeaders(addSecurityHeaders(response2));
          }
        }
        if (ADMIN_ROUTES.some((route) => pathname === route || pathname.startsWith("/api/admin/"))) {
        ...
        try {
          ...
          if (novoToken) {
            ...
            const response2 = jsonResponse({
              ...
              expiraEm,
              novoToken: novoToken.substring(0, 20) + "..."
            });
            return addCorsHeaders(addSecurityHeaders(response2));
          } else {
            const response2 = jsonResponse({
      ...
      if (pathname === "/api/admin/backup" && request.method === "POST") {
        if (!user || !["admin", "coordenador"].includes(user.role)) {
          ...
          return addCorsHeaders(addSecurityHeaders(response3));
        }
        const dados = await request.json();
      ...
      if (pathname === "/api/admin/backup/last" && request.method === "GET") {
        if (!user || !["admin", "coordenador"].includes(user.role)) {
          const response3 = jsonResponse({ error: "Acesso negado" }, 403);
          return addCorsHeaders(addSecurityHeaders(response3));
        }
        const dados = await env.SANTUARIO_KV.get("admin_dados_atual", "json");
        const response2 = jsonResponse({ success: true, dados });
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/admin/backup/list" && request.method === "GET") {
        if (!user || !["admin", "coordenador"].includes(user.role)) {
          const response3 = jsonResponse({ error: "Acesso negado" }, 403);
          return addCorsHeaders(addSecurityHeaders(response3));
        }
        const lista = await env.SANTUARIO_KV.list({ prefix: "backup_admin_" });
        const backups = lista.keys.map((key) => ({
          chave: key.name,
          data: key.name.replace("backup_admin_", "").split("_")[0]
        })).sort((a, b) => b.data.localeCompare(a.data));
        const response2 = jsonResponse({ success: true, backups });
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname.startsWith("/api/admin/backup/restore/") && request.method === "POST") {
        if (!user || !["admin", "coordenador"].includes(user.role)) {
          const response3 = jsonResponse({ error: "Acesso negado" }, 403);
          return addCorsHeaders(addSecurityHeaders(response3));
        }
        const chave = pathname.replace("/api/admin/backup/restore/", "");
        const backup = await env.SANTUARIO_KV.get(chave, "json");
        if (!backup) {
          const response3 = jsonResponse({ error: "Backup n\xE3o encontrado" }, 404);
          return addCorsHeaders(addSecurityHeaders(response3));
        }
        await env.SANTUARIO_KV.put("admin_dados_atual", JSON.stringify(backup.dados));
        const response2 = jsonResponse({ success: true, dados: backup.dados });
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      ...
      if (pathname === "/api/auth/register" && request.method === "POST") {
        const response2 = await fielRegisterRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      ...
      if (pathname === "/api/auth/setup-2fa" && request.method === "POST") {
        const response2 = await fielSetup2faRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      ...
      if (pathname === "/api/auth/admin/verify-pin" && request.method === "POST") {
        const response2 = await fielVerifyPinRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/admin/verify-2fa" && request.method === "POST") {
        const response2 = await fielVerify2faRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/admin/reenviar-pin" && request.method === "POST") {
        const response2 = await fielReenviarPinRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/admin/verificar" && request.method === "GET") {
        const response2 = await adminVerificarRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      ...
      if (pathname === "/api/auth/admin/esqueci-senha" && request.method === "POST") {
        const response2 = await fielEsqueciSenhaRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/admin/confirmar-reset-senha" && request.method === "POST") {
        const response2 = await fielConfirmarResetSenhaRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/admin/reset-2fa-backup" && request.method === "POST") {
        const response2 = await fielReset2faBackupRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/admin/solicitar-reset-2fa" && request.method === "POST") {
        const response2 = await fielSolicitarReset2faRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/admin/confirmar-reset-2fa" && request.method === "POST") {
        const response2 = await fielConfirmarReset2faRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/fiel/login" && request.method === "POST") {
        const response2 = await fielLoginRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/fiel/register" && request.method === "POST") {
        const response2 = await fielRegisterRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/fiel/verificar" && request.method === "GET") {
        const response2 = await fielVerificarRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/fiel/verify-pin" && request.method === "POST") {
        const response2 = await fielVerifyPinRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/fiel/reenviar-pin" && request.method === "POST") {
        const response2 = await fielReenviarPinRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/fiel/verify-2fa" && request.method === "POST") {
        const response2 = await fielVerify2faRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/fiel/esqueci-senha" && request.method === "POST") {
        const response2 = await fielEsqueciSenhaRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      if (pathname === "/api/auth/fiel/confirmar-reset-senha" && request.method === "POST") {
        const response2 = await fielConfirmarResetSenhaRoute(request, env);
        return addCorsHeaders(addSecurityHeaders(response2));
      }
      ...
      if (pathname === "/api/prayer" && request.method === "POST") {
        try {
          ...
          console.error("Erro ao processar pedido de ora\xE7\xE3o:", error);
      ...
      if (pathname === "/api/contato/pascom/enviar" && request.method === "POST") {
        try {
          ...
          console.log("\u{1F4E7} Enviando mensagem da Pascom");
          ctx.waitUntil(sendContactConfirmationEmail(env, body));
          ctx.waitUntil(sendContactNotificationToSecretariat(env, body));
          const response2 = jsonResponse({ success: true, message: "Mensagem enviada com sucesso para a Pascom!" });
          return addCorsHeaders(addSecurityHeaders(response2));
        } catch (error) {
          console.error("\u274C Erro ao enviar mensagem para Pascom:", error);
          const response2 = jsonResponse({ success: false, message: "Erro ao enviar mensagem" }, 500);
          return addCorsHeaders(addSecurityHeaders(response2));
        }
      }
        ...
        try {
          ...
          const emailResponse = await fetch("https://api.resend.com/emails", {
            ...
            body: JSON.stringify({
              from: "Santuario de Fatima <noreply@mail.santuariodefatima.com.br>",
              to: [body.to],
              subject: body.subject || "Teste do Santuario de Fatima",
              html: body.html || "<p>Teste de email</p>"
            })
          });
          const data = await emailResponse.json();
          if (emailResponse.ok) {
            const response3 = jsonResponse({ success: true, message: "Email enviado com sucesso!", id: data.id });
            return addCorsHeaders(addSecurityHeaders(response3));
          }
          const response2 = jsonResponse({ success: false, error: data }, 500);
          return addCorsHeaders(addSecurityHeaders(response2));
        } catch (error) {
          const response2 = jsonResponse({ success: false, error: error.message }, 500);
          return addCorsHeaders(addSecurityHeaders(response2));
      ...
      if (pathname === "/api/dados" && request.method === "GET") {
        try {
          ...
          console.error("Erro ao buscar dados p\xFAblicos:", error);
          const response2 = jsonResponse({
            success: true,
            dados: { carrossel: [], momentosLiturgicos: [], popups: [], recados: [], horariosMissas: [] }
          });
      ...
      if (pathname === "/api/importar-dados" && request.method === "POST") {
        if (!user || !["admin", "coordenador"].includes(user.role)) {
          const response2 = jsonResponse({ error: "Acesso negado" }, 403);
          return addCorsHeaders(addSecurityHeaders(response2));
        }
        try {
          const body = await request.json();
          console.log("\u{1F4E6} Recebendo dados para importa\xE7\xE3o...");
          if (body.carrossel?.length) {
            await salvarComBackup(env, env.KV_FILES, "santuario_carrossel", body.carrossel);
          }
          if (body.popups?.length) {
            await salvarComBackup(env, env.KV_FILES, "santuario_popups", body.popups);
          }
          if (body.recados?.length) {
            await salvarComBackup(env, env.KV_FILES, "santuario_recados", body.recados);
          }
          if (body.horariosMissas?.length) {
            await salvarComBackup(env, env.KV_MISSAS, "horariosMissas", body.horariosMissas);
          }
          if (body.momentosLiturgicos?.length) {
            await salvarComBackup(env, env.KV_LITURGIA, "momentos", body.momentosLiturgicos);
          }
          const response2 = jsonResponse({ success: true, message: "Dados importados com sucesso \u{1F680}" });
          return addCorsHeaders(addSecurityHeaders(response2));
        } catch (err) {
          console.error("\u274C Erro ao importar:", err);
          const response2 = jsonResponse({ success: false, error: err.message }, 500);
          return addCorsHeaders(addSecurityHeaders(response2));
        }
      }
      ...
      if (pathname === "/api/liturgia/debug" && request.method === "GET") {
        try {
          const hoje = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
          const [y, m, d] = hoje.split("-");
          const dataRailway = `${d}/${m}/${y}`;
          const res = await fetch(`https://liturgia.up.railway.app/?data=${dataRailway}`, {
            headers: { "User-Agent": "Mozilla/5.0", "Accept": "application/json" }
          });
          const json = await res.json();
          const response2 = jsonResponse({ dataEnviada: dataRailway, railwayRaw: json });
          return addCorsHeaders(addSecurityHeaders(response2));
        } catch (err) {
          const response2 = jsonResponse({ erro: err.message }, 500);
          return addCorsHeaders(addSecurityHeaders(response2));
        }
      }
  ...
  // ============================================
  // ⏰ TAREFA AGENDADA (roda a cada 45 dias)
  // ============================================
  async scheduled(event, env, ctx) {
    console.log("\u23F0 Executando tarefas agendadas...");
    await cleanupOldCandles(env);
    await backupOldPrayers(env);
    console.log("\u{1F504} Verificando token do Instagram para renova\xE7\xE3o...");
    const tokenAtual = await getInstagramToken(env);
    const expiresAt = await env.KV_INSTAGRAM?.get("token_expires_at");
    let precisaRenovar = true;
    if (expiresAt) {
      const diasRestantes = Math.ceil((parseInt(expiresAt) - Date.now()) / 864e5);
      console.log(`\u{1F4C5} Token atual expira em ${diasRestantes} dias`);
      precisaRenovar = diasRestantes < 40;
      if (precisaRenovar) {
        console.log(`\u26A0\uFE0F Token expira em ${diasRestantes} dias (< 40), renovando...`);
      } else {
        console.log(`\u2705 Token ainda tem ${diasRestantes} dias (> 40), n\xE3o precisa renovar agora.`);
      }
    }
    if (precisaRenovar && tokenAtual) {
      ...
      if (novoToken) {
        ...
        console.error("\u274C Falha na renova\xE7\xE3o autom\xE1tica do token");
      }
    } else if (!tokenAtual) {
      console.error("\u274C Nenhum token encontrado no KV. Configure manualmente primeiro.");
    }
  }
};
...
// .wrangler/tmp/bundle-w8Yp4N/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-w8Yp4N/middleware-loader.entry.ts
